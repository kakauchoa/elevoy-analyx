...
function ModalPlano({
  ...
  async function salvar(e: React.FormEvent) {
    ...
    setSalvando(true);
    try {
      const res = await fetch(`/api/admin-master/usuarios/${gestor.id}/plano`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ plano, contasMaximas: Number(contasMaximas), assinaturaAtiva }),
      });
      const data = (await res.json()) as {
        id: string; plano: Plano; contasMaximas: number; assinaturaAtiva: boolean; erro?: string;
      };
        ...
        <form onSubmit={(e) => void salvar(e)} className="flex flex-col gap-4">
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <button
              ...
              className={`relative inline-flex h-5 w-9 rounded-full border-2 border-transparent transition-colors ${assinaturaAtiva ? "bg-black" : "bg-gray-200"}`}
            >
...
function ModalContas({
  ...
  const [todasContas, setTodasContas] = useState<ContaSimples[]>([]);
  ...
  useEffect(() => {
    async function carregar() {
      const [contasRes, vinculosRes] = await Promise.all([
        fetch("/api/admin-master/contas"),
        fetch(`/api/admin-master/usuarios/${gestor.id}/contas`),
      ]);
      if (contasRes.ok) setTodasContas(await contasRes.json() as ContaSimples[]);
      if (vinculosRes.ok) setSelecionadas(new Set(await vinculosRes.json() as string[]));
      setCarregando(false);
    }
    void carregar();
  }, [gestor.id]);
  ...
  async function salvar() {
    ...
    try {
      ...
      const [contasRes] = await Promise.all([
        ...
        // Atualiza permissão "Clientes" conforme seleção
        fetch(`/api/admin-master/usuarios/${gestor.id}/permissoes`).then(async (r) => {
          if (!r.ok) return;
          const atual = (await r.json()) as string[];
          const novas = contaIds.length > 0
            ? [...new Set([...atual, "Clientes"])]
            : atual.filter((s) => s !== "Clientes");
          return fetch(`/api/admin-master/usuarios/${gestor.id}/permissoes`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
      ...
      const permissoesRes = await fetch(`/api/admin-master/usuarios/${gestor.id}/permissoes`);
  ...
  const contasFiltradas = todasContas.filter((c) =>
    !busca || c.nomeCliente.toLowerCase().includes(busca.toLowerCase())
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 flex flex-col gap-4">
        ...
        {!carregando && todasContas.length > 5 && (
          <input
            type="text"
            placeholder="Buscar conta..."
            value={busca}
            onChange={(e) => setBusca(e.target.value)}
            className="border border-[#e5e5e5] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black"
          />
        )}
        ...
        {carregando ? (
          ...
          <p className="text-sm text-gray-500 text-center py-4">Nenhuma conta encontrada.</p>
        ) : (
          <div className="flex flex-col gap-2 max-h-64 overflow-y-auto">
            {contasFiltradas.map((c) => (
              <label key={c.id}
                className="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-[#e5e5e5] cursor-pointer hover:bg-gray-50 transition-colors">
                <input
                  type="checkbox"
        ...
        <p className="text-xs text-gray-400">
          {selecionadas.size === 0
            ...
            : `${selecionadas.size} conta(s) — permissão de Clientes será concedida automaticamente`}
        </p>
...
export default function AdminPainelPage() {
  ...
  useEffect(() => {
    fetch("/api/admin-master/gestores")
      .then(async (r) => {
  ...
  async function logout() {
    ...
    await fetch("/api/admin-master/auth", { method: "DELETE" });
  ...
  async function toggleSessoes(gestor: GestorAdmin) {
    ...
    try {
      ...
      const res = await fetch(`/api/admin-master/usuarios/${gestor.id}/permissoes`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ secoes: novas }),
      });
      if (res.ok) {
        setGestores((prev) => prev.map((g) => g.id === gestor.id ? { ...g, permissoes: novas } : g));
      }
    } finally {
  ...
  const filtrados = gestores.filter((g) => {
    const matchBusca = !busca || g.nome.toLowerCase().includes(busca.toLowerCase()) || g.email.toLowerCase().includes(busca.toLowerCase());
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <header className="bg-white border-b border-[#e5e5e5] px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">Elevoy Analyx</p>
          </div>
        </div>
        <button
          onClick={() => void logout()}
          disabled={fazendoLogout}
          className="text-xs font-medium text-gray-600 border border-[#e5e5e5] rounded-lg px-3 py-1.5 hover:bg-gray-50 transition-colors disabled:opacity-50"
        >
      ...
      <div className="flex-1 p-6 max-w-5xl mx-auto w-full space-y-5">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Controle de Gestores</h2>
          <p className="text-sm text-gray-500 mt-0.5">Gerencie planos, limites e acessos de todos os gestores</p>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {(["free", "basico", "intermediario", "personalizado"] as Plano[]).map((p) => (
              <button
                ...
                className={`border rounded-xl px-4 py-3 text-left transition-colors ${filtroPlano === p ? "border-black bg-gray-50" : "border-[#e5e5e5] hover:border-gray-300"}`}
              >
        ...
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 border border-[#e5e5e5] rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-black bg-white"
          />
          {(busca || filtroPlano) && (
            <button onClick={() => { setBusca(""); setFiltroPlano(""); }}
              className="text-sm text-gray-500 hover:text-gray-900 border border-[#e5e5e5] rounded-lg px-4 py-2 bg-white">
              Limpar
            </button>
          )}
        </div>
                ...
                <div key={g.id}
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {g.assinaturaAtiva ? (
                        <span className="shrink-0 text-[10px] font-medium px-2 py-0.5 rounded-full bg-green-50 text-green-700 border border-green-200">Ativo</span>
                      ) : (
                        <span className="shrink-0 text-[10px] font-medium px-2 py-0.5 rounded-full bg-red-50 text-red-600 border border-red-200">Inativo</span>
                      )}
                      {g.permissoes.includes("Clientes") && (
                        <span className="shrink-0 text-[10px] font-medium px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-200">Contas</span>
                      )}
                      {temSessoes && (
                        <span className="shrink-0 text-[10px] font-medium px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">Sessões WPP</span>
                      )}
                    </div>
                    <p className="text-xs text-gray-500 mt-0.5 truncate">{g.email}</p>
                    <p className="text-xs text-gray-400 mt-1">
                      {g.totalContas}/{g.contasMaximas} contas usadas
                      {g.assinaturaVenceEm && <> · Renova {new Date(g.assinaturaVenceEm).toLocaleDateString("pt-BR")}</>}
                      <> · Desde {new Date(g.criadoEm).toLocaleDateString("pt-BR")}</>
                    </p>
                  </div>

                  <div className="shrink-0 flex items-center gap-2">
                    <button onClick={() => setGestorContas(g)}
                    ...
                    <button
                      ...
                      className={`text-xs font-medium border rounded-lg px-3 py-1.5 transition-colors disabled:opacity-50 ${
                        temSessoes ? "text-emerald-700 border-emerald-200 bg-emerald-50 hover:bg-emerald-100" : "text-gray-700 border-[#e5e5e5] hover:bg-gray-50"
                      }`}
                    >
                      {toggling ? "…" : temSessoes ? "Sessões ✓" : "Sessões"}
                    </button>
                    <button onClick={() => setGestorEditando(g)}
                      className="text-xs font-medium text-gray-700 border border-[#e5e5e5] rounded-lg px-3 py-1.5 hover:bg-gray-50 transition-colors">
                      Plano
                    </button>
                  </div>
      ...
      {gestorEditando && (
        <ModalPlano gestor={gestorEditando} onClose={() => setGestorEditando(null)} onSalvo={handleSalvoPlano} />
      )}
      {gestorContas && (
        <ModalContas
          gestor={gestorContas}
...
function KpiCard({ titulo, valor, cor = "text-gray-900" }: { titulo: string; valor: string | number; cor?: string }) {
  return (
